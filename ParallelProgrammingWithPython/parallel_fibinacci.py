...
def fibonacci_task(condition):
    with condition:
        ...
        logger.info(
            "[%s] fibonacci of key [%d] with result [%d]" % (threading.current_thread().name, value, fibo_dict[value]))
...
threads = [threading.Thread(daemon=True, target=fibonacci_task, args=(queue_condition,)) for i in range(6)]
